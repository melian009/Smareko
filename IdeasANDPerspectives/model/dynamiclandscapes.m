...
%assumptions: Rgn with random-variable threshold 
%+ multitrophic dynamics with ecological drift
%Feb-Mar 2014
%De Santana, Klecka and Melian (2014)
%---------------------------------------------------------------
for ri = 1:100;
G = unidrnd(10,1,1);%Generations could be variable
s = rand;%prob to be a static landscape
S = 100;J = 100;%S sites and J inds. per site
%Resources
newspeciesR = 1;R = ones(S,J);mr = unifrnd(0.2,0.7,1);vr = unifrnd(0.0001,0.01,1);
%Consumers
newspeciesH = 1;H = ones(S,J);mh = unifrnd(0.2,0.7,1);vh = unifrnd(0.0001,0.01,1);
%Predators
newspeciesP = 1;P = ones(S,J);mp = unifrnd(0.2,0.7,1);vp = unifrnd(0.0001,0.01,1);
